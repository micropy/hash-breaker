...
def dehash_sha512(hash_str: str, salt_str: str, chunk):
    start = time.time()
    ...
    for password in chunk:
        ...
        # try three different combinations of password and salt for sha512 hash
        combined1 = sha512(password_encoded + salt_encoded).hexdigest()
        combined2 = sha512(salt_encoded + password_encoded).hexdigest()
        combined3 = sha512(sha512(password_encoded).hexdigest().encode() + salt_encoded).hexdigest()
